...
def fast_test_incremental_metagraph_graphfd(config, base, loaders, current_step, if_test_forget=True):
...
def save_and_fast_test_incremental_metagraph_graphfd(config, base, loaders, current_step, current_epoch,if_test_forget=True):
    # using Cython test during train
    # return mAP, Rank-1
    base.set_all_model_eval()
    print(f'****** start perform fast testing! ******')
